...
| 4.16 | 🟡 **Chaînage sauvegarde ↔ cycle de vie VM — en place, contrat API à renseigner** | `deletevm` et `inplace-upgrade` embarquent désormais un rôle `preflight_backup` qui interroge le serveur de sauvegarde (Veeam VBR ou NetBackup) en lecture seule et refuse l'opération sans point de restauration **réussi et récent** pour la VM. Il échoue fermé : activé par défaut, une configuration absente bloque au lieu de laisser passer. Les jobs en échec, les points d'une autre VM et un dernier point trop ancien sont tous rejetés ; l'âge est mesuré sur le dernier point *réussi*, pas sur le plus récent tout court. La dérogation (`*_backup_allow_missing=true`) existe mais est explicite et tracée dans l'artefact AAP (`waived` / `stale-waived`). **Reste à la charge de l'exploitant** : le contrat API par release (endpoint, en-tête de version, noms de champs de la réponse), sur le même modèle que les projets d'appliance. |
