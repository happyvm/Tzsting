...
| Configuration Day-1 | 🟡 | 🟡 | ❌ | ✅ `synergy-conf`, `purestorage-conf`, `dxi-conf`, `storeonce-conf` |
...
**configuration initiale des appliances**, et **absent sur tout ce qui touche
l'hôte hyperviseur lui-même, le firmware et le patch courant**.
...
| 1.2 | **Aucune opération réseau sur la VM** | Pas d'ajout/suppression de vNIC, pas de changement de port group / VM Network / VLAN côté VM. `synergy-vlan-*` agit sur OneView, pas sur la VM. | Une demande « changer la VM de VLAN » reste manuelle |
...
**C'est l'axe le plus vide du dépôt.** Aucun projet ne cible l'hôte ESXi, le
nœud Hyper-V, le cluster, ni les appliances de management (vCenter, SCVMM).
Les hôtes n'existent que comme *cible de délégation* (`hyperv_hypervisor`,
`scvmm_management`) ou comme *source de recherche* de VM.

### Écarts structurants

| # | Écart | Détail |
|---|---|---|
| 3.1 | **Pas de configuration d'hôte ESXi** | NTP, syslog, DNS, services (SSH, shell), utilisateurs locaux, mode de verrouillage, règles de pare-feu, politique d'alarme, `advanced settings`, profil d'énergie. À noter : ce périmètre **est** couvert pour les baies et les appliances de sauvegarde — l'hyperviseur est le seul équipement de la chaîne à ne pas l'être. |
...
| 3.5 | **Pas de configuration des appliances de management** | Aucun équivalent de `*-conf` pour vCenter/VCSA ni pour SCVMM : NTP, AD/SSO, rôles et permissions, certificats, alarmes, sauvegarde de configuration. |
...
| 4.12 | **Pas de cycle de vie de la baie** | Mise à jour Purity, certificats, SNMP/alerting, support/Pure1, mise en service et décommissionnement d'une baie. |
...
- `vmware_validate_certs: false` est la valeur par défaut dans **6 projets
  sur 6** touchant vCenter, alors que les projets appliance récents
  (`dxi`, `storeonce`) sont à `true`. Aligner sur `true` par défaut et
  documenter le déploiement de la chaîne interne dans l'EE.
...
- **6 workflows pour 15 projets.** Aucune CI pour `inplace-upgrade`,
  `purestorage-conf`, `purestorage-volhost-create`, `purestorage-volhost-remove`,
  `synergy-conf`, `synergy-get`, `synergy-vlan-add`, `synergy-vlan-remove`,
  `quantum-dxi-conf`, `hpe-storeonce-conf` — soit **10 projets sans lint ni
  syntax-check automatisés**, dont tous les projets matériel.
...
| `purestorage-conf` | Certificats, SNMP/alerting, seuils de capacité | S |
...
| Tous | CI (yamllint + ansible-lint + syntax-check) sur les 10 projets non couverts | S |
...
1. **CI sur les 10 projets non couverts** (S). Les projets matériel, les plus
   destructeurs, n'ont aucun garde-fou automatisé aujourd'hui.
...
9. **Configuration d'hôte ESXi** (M, écart 3.1) et **inventaire/conformité
   d'hôte** (M, écart 3.8).
...
1. **Le dépôt doit-il couvrir l'hôte hyperviseur ?** S'il s'agit d'un choix
   (hôtes gérés par une autre équipe ou un autre outil), le documenter dans
   `ANSIBLE.md` comme hors périmètre plutôt que de le laisser apparaître
   comme un oubli. Sinon, l'axe 3 est le plus gros chantier du dépôt.
