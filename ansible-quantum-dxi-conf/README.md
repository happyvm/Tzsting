...
DXi** : comptes locaux, intégration Active Directory, NTP et timezone. Le
projet suit la méthodologie du dépôt : inventaire sans appliance statique,
préflight bloquant, secrets Vault/AAP, rôles séparés et artefact `set_stats`.
...
endpoint destructif : les quatre variables `*_endpoint` sont vides et le
préflight refuse l'exécution tant qu'elles ne sont pas définies d'après le
guide API livré avec **la version cible** :
